...
def array6(nums, index):
	if nums==[]:
		...
		return array6(nums[index+1:], index)
...
def array11(nums, index):
	if nums==[]:
		...
		return 1+array11(nums[index+1:],index)
	else:
		return 0+array11(nums[index+1:],index)
...
def array220(nums, index):
	if nums==[] or len(nums)==1:
		...
		return array220(nums[index+1:],index)
...
def strCopies(str, sub, n):
	...
	if str=='':
		...
		return (1+strCopies(str[1:],sub,n-1))>=n
	else:
		return (0+strCopies(str[1:],sub,n-1))>=n
      ...
      for i in range(len(locals()[prob+"_args"])):
        if (type(locals()[prob+"_args"][i]) is str) or (type(locals()[prob+"_args"][i]) is int) or (len(locals()[prob+"_args"][i]) == 1): # function takes one argument
          ...
          if globals()[prob](first, second) == locals()[prob+"_ans"][i]:
              ...
              print "\nWorng!", prob + "(" + str(locals()[prob+"_args"][i]) + ") result:", str(globals()[prob](first, second)), " expected:", str(locals()[prob+"_ans"][i])
        else:    
